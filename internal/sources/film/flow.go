...
const (
	...
	threadBaseURL = "https://boards.4chan.org/tv/thread/"
)
...
func (Flow) Extract(page rfs.Page) ([]rfs.ExtractedItem, error) {
	...
	if err := json.Unmarshal([]byte(page), &doc); err != nil {
		return nil, fmt.Errorf("film: parse catalog: %w", err)
	}
	...
	if !matched {
		return nil, errors.New("film: no matching threads")
	}
	if len(items) == 0 {
		return nil, errors.New("film: no valid threads")
	}
	return items, nil
}

// matchSubject reports whether a catalog subject names the /film/ general.
// The strict "/film/" form avoids false positives on subjects that merely
// contain the letters film.
